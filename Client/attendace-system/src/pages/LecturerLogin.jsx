...
import { loginLecturer } from '../../connectBackend';
...
const LecturerLogin = () => {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    try {
      const response = await loginLecturer({ email, password });
      ...
      <div className="w-full md:w-1/2 max-w-md bg-white rounded-lg shadow-lg p-8">
        ...
        <form className="space-y-5" action="#" onSubmit={handleLogin}>
          ...
          <p className="text-sm text-gray-600 text-center">
            ...
            <Link to="/registerLecturer" className="text-blue-600 hover:underline">
              Sign up
            </Link>
          </p>
        </form>
