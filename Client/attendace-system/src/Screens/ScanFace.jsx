...
import { markAttendance, profile } from "../../connectBackend";
...
const ScanFace = () => {
  ...
  const triggerLockout = () => {
    setIsLockedOut(true);
    lockoutTimer.current = setTimeout(() => setIsLockedOut(false), 2 * 60 * 1000);
  };
  ...
  const startQrScanner = async () => {
    ...
    try {
      ...
      if (!devices || devices.length === 0) return alert("No camera found");
  ...
  // Handle face detection & mark attendance
  const handlePlay = () => {
    ...
    <div className="flex flex-col justify-center items-center mt-10 md:mt-0">
      <section className="flex flex-col justify-center items-center gap-4 p-4">
        ...
        {!code ? (
          ...
          <img src={scanAnimation} alt="Scan Animation" className="w-70 h-70 object-cover" />
        )}

        {!code ? (
          <button className="bg-blue-600 text-white px-4 py-2 rounded" onClick={startQrScanner}>
            Start QR Scan
          </button>
        ) : (
          <button className="bg-[#00294f] text-white font-bold px-4 py-2 rounded w-70" onClick={startVideo}>
            {scanning ? "Scanning..." : "Scan Face"}
          </button>
        )}
      </section>

      {isModalOpen && attendanceInfo && (
        <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-40 z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg text-center max-w-sm">
            <div className="text-green-500 text-5xl mb-2">✔️</div>
            <h2 className="text-xl font-semibold mb-2">{attendanceInfo.name || ""}</h2>
            <p className="text-gray-700">{attendanceInfo.email || attendanceInfo.message}</p>
          </div>
        </div>
      )}
    </div>
